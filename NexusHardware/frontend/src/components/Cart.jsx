    ...
    return (
        <div className="fixed top-20 right-4 w-96 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl p-6 z-50 flex flex-col max-h-[80vh]">
            <div className="flex justify-between items-center border-b border-slate-700 pb-4 mb-4">
                <h2 className="text-xl font-bold text-slate-100">Tu Carrito</h2>
                ...
                <>
                        ...
                        {cartItems.map((item, index) => (
                            <div key={`${item.id}-${index}`} className="flex justify-between items-start bg-slate-800/50 p-3 rounded-lg group">
                                <div className="flex-1 min-w-0 mr-4">
                                    ...
                                    <p className="text-xs text-slate-400">${item.price.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</p>
                                </div>
                                {onRemove && (
                                    <button
                    ...
                    <div className="border-t border-slate-700 pt-4">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <span className="text-2xl font-bold text-blue-400">${total.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                        </div>
