...
const AdminPanel = ({ username }) => {
    ...
    const handleChartClick = (data) => {
        ...
        try {
            ...
            // Info
            doc.setFontSize(10);
            doc.text(`Transacción ID: ${data.id}`, 14, 40);
            ...
            doc.save(`comprobante_${data.id}.pdf`);
    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-bold text-slate-100">Panel de Control</h2>
                    <p className="text-slate-400 text-sm">Bienvenido, {username}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-slate-800 border border-cyan-500/30 p-6 rounded-xl shadow-[0_0_20px_rgba(6,182,212,0.1)] text-center">
                    <h3 className="text-gray-400 text-xs mb-2 uppercase tracking-widest">Ingresos Totales</h3>
                ...
                <div className="bg-slate-800 border border-purple-500/30 p-6 rounded-xl shadow-[0_0_20px_rgba(168,85,247,0.1)] text-center">
                    <h3 className="text-gray-400 text-xs mb-2 uppercase tracking-widest">Ventas Totales</h3>
                ...
                <div className="bg-slate-800 border border-blue-500/30 p-6 rounded-xl text-center">
                    <h3 className="text-gray-400 text-xs mb-2 uppercase tracking-widest">Últimos 7 Días</h3>
                    ...
                    <p className="text-xs text-blue-400">{stats.sales_7d || 0} ventas</p>
                </div>

                <div className="bg-slate-800 border border-blue-500/30 p-6 rounded-xl text-center">
                    <h3 className="text-gray-400 text-xs mb-2 uppercase tracking-widest">Últimos 30 Días</h3>
                    <p className="text-xl font-mono font-bold text-white mb-1">
                        ${stats.revenue_30d?.toLocaleString('es-AR', { minimumFractionDigits: 2 }) || '0.00'}
                    </p>
                    <p className="text-xs text-blue-400">{stats.sales_30d || 0} ventas</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
                    <h3 className="text-lg font-bold text-slate-100 mb-4 border-b border-slate-700 pb-2">Agregar Nuevo Producto</h3>
                    ...
                    <form onSubmit={handleAddProduct} className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-slate-400 text-xs mb-1">Nombre del Producto</label>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-slate-400 text-xs mb-1">Precio (ARS)</label>
                            ...
                            <div>
                                <label className="block text-slate-400 text-xs mb-1">Stock (Cant.)</label>
                ...
                {/* Gráfico de Ventas */}
                <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
                    <h3 className="text-lg font-bold text-slate-100 mb-4 border-b border-slate-700 pb-2">Transacciones Recientes</h3>
                    <p className="text-xs text-slate-400 mb-2">Click en una barra para descargar comprobante</p>
                    <div className="h-64">
            ...
            <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                <div className="p-6 border-b border-slate-700 flex justify-between items-center">
                    <div>
                        <h3 className="text-lg font-bold text-slate-100">Motor de Optimización de Stock</h3>
                        <p className="text-xs text-slate-400">Potenciado por Simulación C-Logic</p>
                        <p className="text-[10px] text-slate-500 mt-1 italic max-w-md">
                            *Modo Demo: Las ventas promedio y tiempos de entrega son simulados para demostrar la lógica sin requerir meses de datos históricos.
                        </p>
                    </div>
                    <span className="px-2 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs font-bold rounded">EN LÍNEA</span>
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-900/50 text-slate-400 text-xs uppercase font-semibold">
                            <tr>
                                ...
                                <th className="p-4">Ventas Prom. Mensual</th>
                                <th className="p-4">Tiempo de Entrega (Días)</th>
                                <th className="p-4 text-right">Sugerencia Reposición</th>
