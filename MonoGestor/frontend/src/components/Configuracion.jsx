...
const Configuracion = () => {
    ...
    const handleReset = () => {
        if (!window.confirm("¡ATENCIÓN! Esto eliminará TODOS los movimientos, el stock y las categorías personalizadas. No se puede deshacer. ¿Estás seguro?")) {
            return;
        }

        if (!window.confirm("¿Seguro, seguro? Esta es la última advertencia.")) {
            return;
        }
        ...
        fetch('http://localhost:8001/reset-db', { method: 'POST' })
            ...
            .then(data => {
                ...
                setLoading(false);
                alert("Base de datos reiniciada. La aplicación se recargará.");
